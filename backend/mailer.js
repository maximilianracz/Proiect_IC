...
async function sendDonationRequestReminderEmail(to, donationDetails) {
  console.log('🔔 DEBUG: Începe procesul de trimitere email reminder pentru cerere de donație');
  console.log('📧 DEBUG: Detalii email:', {
    to,
    donationDetails,
    timestamp: new Date().toISOString()
  ...
  const mailOptions = {
    from: "centru.donatii@gmail.com",
    to,
    subject: "📦 Reminder: Colectare donație în 3 ore!",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #2c3e50;">Reminder Colectare Donație</h2>
        <p>Salut,</p>
        <p>Acesta este un reminder că ai o cerere de donație care trebuie colectată în 3 ore:</p>
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          <ul style="list-style: none; padding: 0;">
            ...
            <li>📦 Produse:</li>
            <ul>
              ${donationDetails.produse.map(produs => `
                <li>${produs.tip} (${produs.marime}) x${produs.cantitate}</li>
              `).join('')}
            </ul>
          </ul>
        </div>
        <p>Te rugăm să te asiguri că toate produsele sunt pregătite pentru colectare.</p>
        <p>Îți mulțumim pentru implicare!</p>
        <hr style="border: 1px solid #eee; margin: 20px 0;">
        <p style="color: #666; font-size: 12px;">Acesta este un mesaj automat. Te rugăm să nu răspunzi la acest email.</p>
      </div>
    `
...
module.exports = { 
  ...
  sendReminderEmail,
  sendDonationRequestReminderEmail 
};
